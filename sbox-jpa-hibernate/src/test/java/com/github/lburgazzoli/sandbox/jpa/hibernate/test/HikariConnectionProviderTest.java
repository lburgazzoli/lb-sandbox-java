 ...
 */
package com.github.lburgazzoli.sandbox.jpa.hibernate.test;

import com.github.lburgazzoli.sandbox.jpa.hibernate.connection.HikariConfigurationUtil;
...
public class HikariConnectionProviderTest {
    ...
    private Properties getHibernateProperties() {
        ...
        props.setProperty("hibernate.hikari.dataSourceClassName" , org.hsqldb.jdbc.JDBCDataSource.class.getName());
    ...
    public void testHikariConnectionProvider() {
        ...
        // Validate HikariConfig
        hc.validate();

        Assert.assertFalse(hc.isAutoCommit());
        Assert.assertEquals(hc.getDataSourceClassName(), hbp.getProperty("hibernate.hikari.dataSourceClassName"));
        ...
        for(String key : hbp.stringPropertyNames()) {
            ...
            if(key.startsWith(HikariConfigurationUtil.CONFIG_PREFIX_DATASOURCE)) {
                Assert.assertEquals(
                    hbp.getProperty(key),
                    hc.getDataSourceProperties().getProperty(key.substring(HikariConfigurationUtil.CONFIG_PREFIX_DATASOURCE.length()))
                );
            }
        }
    }
